...
object IORuntime {
  ...
  /** Useful to create all needed machinery to properly work with the cats-effect runtime.
    *
    * You really should instantiate this once per app, therefore also the Later return type, to properly hammer the
    * point home.
    */
  ...
  /** Use UnsafePools.defaultMainExecutionContext, then pass it here, or use defaultMainRuntime to instantiate all basic
    * machinery.
    */
  ...
  /** Most likely you need only defaultMainRuntime, because there's little reason to expose the EC underlying your
    * ContextShift and Timer.
    *
    * But in the cases you need that, this method behaves like defaultMainRuntime, but it also gives you the underlying
    * main thread pool
    */
